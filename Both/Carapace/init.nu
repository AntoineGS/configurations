# Based on `carapace _carapace nushell` (v1.7.3) output, with the hardcoded
# machine-specific PATH prepend replaced by a portable per-platform computation.
let carapace_bin = if $nu.os-info.name == "windows" {
  $env.APPDATA | path join "carapace" "bin"
} else {
  ($env.XDG_CONFIG_HOME? | default ("~/.config" | path expand)) | path join "carapace" "bin"
}
