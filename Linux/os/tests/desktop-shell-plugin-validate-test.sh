...
for id in unsafe desktop.clock omarchy.clock; do expect_reject "unsafe-${id//./-}" "plugin id" env ID="$id" bash -c 'jq --arg id "$ID" ".id=\$id" "$TEST_DIRECTORY/manifest.json" >"$TEST_DIRECTORY/manifest.tmp" && mv "$TEST_DIRECTORY/manifest.tmp" "$TEST_DIRECTORY/manifest.json"'; done
