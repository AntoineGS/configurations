...
**Arguments:** `<description of failing tests or test file paths>`
...
4. **Use only local agents.** All `agent` references use agents bundled with this plugin or `task`. No cross-plugin dependencies.
5. **Never enter plan mode autonomously.** Do NOT use plan mode. Execute directly.
