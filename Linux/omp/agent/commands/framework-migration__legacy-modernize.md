...
**Arguments:** `<legacy codebase path or description> [--strategy parallel-systems|big-bang|by-feature|database-first|api-first]`
...
3. **Stop at checkpoints.** When you reach a `PHASE CHECKPOINT`, you MUST stop and wait for explicit user approval before continuing. Use the `ask` tool with clear options.
...
5. **Use only local agents.** All `agent` references use agents bundled with this plugin or `task`. No cross-plugin dependencies.
6. **Never enter plan mode autonomously.** Do NOT use plan mode. This command IS the plan — execute it.
...
Use the `task` tool with agent="framework-migration__legacy-modernizer":
...
Use the `task` tool with agent="framework-migration__architect-review":
...
Use the `task` tool with agent="task":
...
Detect the target language/stack from the legacy assessment. Use the `task` tool with agent="task", providing role context matching the target stack:
