...
BarWidget {
  ...
  // Keep a live opener for each submenu level. A child entry is owned by its
  // parent opener model, so collapsing the stack would destroy the entry being
  // displayed before the next level can render.
  ...
  clip: false
  implicitWidth: drawerCount > 0 ? (root.vertical ? root.barSize : trayContent.implicitWidth) : 0
  implicitHeight: drawerCount > 0 ? (root.vertical ? trayContent.implicitHeight : root.barSize) : 0
