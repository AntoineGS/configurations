...
Panel {
  ...
  function statusText(p) {
    if (!p) return ""
    var messages = []
    if (p.stale === true) messages.push("Showing the last valid record; refresh is stale.")
    if (String(p.error || "") !== "") messages.push(String(p.error))
    if (String(p.usageStatusText || "") !== "") messages.push(String(p.usageStatusText))
    if (String(p.authHelpText || "") !== "") messages.push(String(p.authHelpText))
    return messages.join(" ")
  }

  function weekPeak(p) {
    var days = p ? (p.dailyUsage || []) : []
    var peak = 0
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          // ---------- Status ----------
          BorderSurface {
            visible: root.statusText(root.provider) !== ""
            width: parent.width
            ...
            Text {
              ...
              text: root.statusText(root.provider)
              color: root.dim
              font.family: root.fontFamily
          ...
          Column {
            id: usageSection
            visible: !!root.provider && root.provider.dailyUsage && root.provider.dailyUsage.length > 0
            width: parent.width
            spacing: Style.spacing.md

            readonly property var days: root.provider ? (root.provider.dailyUsage || []) : []
          ...
          Column {
            ...
            PanelSectionHeader {
              width: parent.width
              text: "TOKENS BY MODEL · LAST 7 DAYS"
              foreground: root.foreground
              fontFamily: root.fontFamily
            }
