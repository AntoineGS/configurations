...
QtObject {
  ...
  readonly property QtObject menu: QtObject {
    ...
    property color selectedBackground: root.composed(
      "menu.selected-background", "menu.selected-background-alpha", root.foreground, 0.08)
    property color selectedText: root.pick("menu.selected-text", root.accent)
    property color selectedBorder: root.composed(
      "menu.selected-border", "menu.selected-border-alpha", root.foreground, 0.0)
  }
  ...
  // Style. User keys win, so a machine-level shell.toml survives theme
  // switches (which replace only themeShellValues).
  function mergeShell() {
    var merged = {}
  ...
  // Local font settings are watched so they take effect live without
  // restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/quickshell/desktop-shell/config/shell.toml"
    watchChanges: true
    printErrors: false
