...
QtObject {
  ...
  // component follows the system fontconfig alias.
  ...
  // alias path remains effective), but read `resolvedFontFamily` when you want
  // to display what's drawing.
  ...
  // DESKTOP_SHELL_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("DESKTOP_SHELL_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name and rerun on file
  // change so manual edits propagate too.
  ...
  // A local window-gap toggle creates/removes this flag file. Hyprland reloads
  // its config when sourced files change, then hyprctl reflects the value.
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/desktop-shell/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
