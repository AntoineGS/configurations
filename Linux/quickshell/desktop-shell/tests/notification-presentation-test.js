...
const serviceSource = require("node:fs").readFileSync(require.resolve("../plugins/notifications/Service.qml"), "utf8")
assert.match(serviceSource, /snapshot\.duration\s*=\s*durationFor/)
assert.match(serviceSource, /snapshot\.remainingLifetime\s*=\s*durationFor/)
assert.match(serviceSource, /updated\.presentationSource = livePersistenceSources/)
assert.match(serviceSource, /service\.dispatchPresentation\(previousIdentity === ""/)
assert.match(serviceSource, /String\(outputName\) !== service\.presentationState\.visual\.output/)
assert.match(serviceSource, /nextMonotonicTimestamp\(service\.latestLiveGeneration, Date\.now\(\)\)/)
